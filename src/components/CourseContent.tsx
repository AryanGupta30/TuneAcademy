...
const CourseContent: React.FC<NavbarProps> = ({ onSelectVideo }) => {
  ...
  return (
    <div className="min-h-screen bg-black/[0.96] antialiased flex flex-col md:w-64 text-white h-auto md:h-screen p-4 overflow-y-auto">
      <div className="flex flex-col flex-1 space-y-4">
        <div className="relative">
          <button
            className={`py-2 md:py-4 px-4 my-2 md:my-4 w-full hover:bg-gray-900 ${
              openTab === "basic" ? "bg-slate-800" : ""
            }`}
          ...
          {openTab === "basic" && (
            <div className="flex flex-col space-y-2 mt-2 py-2 md:py-4 px-4 my-2 md:my-4 pl-8 md:pl-16 w-full">
              {buttons.map((buttonIndex) => (
                <button
                  key={buttonIndex}
                  onClick={() => selectVideo(openTab, buttonIndex)}
                  className={`py-1 md:py-2 px-4 w-full hover:bg-gray-600 ${
                    selectedVideo === buttonIndex ? "bg-slate-800" : ""
                  }`}
                >
                  Video {buttonIndex}
                </button>
        ...
        <div className="relative">
          <button
            className={`py-2 md:py-4 px-4 my-2 md:my-4 w-full hover:bg-gray-900 ${
              openTab === "intermediate" ? "bg-slate-800" : ""
            }`}
